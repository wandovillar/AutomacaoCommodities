...
# Função para coletar dados de Milho


def coletarDadosMilho():
    ...
    navegador.get("https://br.investing.com/commodities/us-corn-historical-data?cid=964522")
...
# Chama a função para coletar os dados de Milho
dados_milho = coletarDadosMilho()

if not dados_milho.empty:
    ...
    try:
        ...
        dados_novos = dados_milho[~dados_milho.isin(
        ...
        book = load_workbook(filename=nome_planilha_saida)
        sheet = book.active
        sheet.title = abaMilho
        linha_inicial = 1
        dados_novos = dados_milho
    ...
    for row in dados_novos.itertuples(index=False):
        ...
        print(
            f"Planilha '{nome_planilha_saida}' criada e dados adicionados na aba '{abaMilho}'.")
else:
